...
**Read-only** — no moves without user approval (then use normal refactor or `kenmark-repo-hygiene` for clutter).
...
If findings are significant, recommend a short addition to `brain/kb/01-architecture.md` (do not write unless user approves KB update — or use **`kenmark-repo-kb`**).
...
Optional: `brain/reports/kenmark-repo-structure-YYYY-MM-DD.md`
...
| Situation | Prefer |
| --- | --- |
| Random clutter, dumps, secrets filenames | `kenmark-repo-hygiene` |
| KB architecture doc update | `kenmark-repo-kb` |
| Dependency overlap (multiple UI libs) | `kenmark-repo-deps` |
