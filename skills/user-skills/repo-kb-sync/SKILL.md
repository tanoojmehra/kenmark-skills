...
**`commit-push`** enforces KB updates at commit time; this skill is the **how**.

---

## Prerequisite

If `brain/kb/` does not exist, run **`init-brain`** first (creates `00`–`11`, `features/`, `decisions/`).

```bash
test -d brain/kb && echo "kb ok" || echo "run init-brain first"
```
...
| Situation | Prefer |
| --- | --- |
| First-time brain scaffold | `init-brain` |
| Commit with KB gate | `commit-push` |
| Docs/README accuracy | `repo-docs-audit` |
| File clutter | `repo-hygiene` |
