...
Pair with **skills-init** for first-time setup (humans: interactive wizard; agents: `init -y`), **skills-install-recommended** when adding curated packs after cleanup, and **skills-update** when refreshing Kenmark or recommended installs.
...
- `impeccable`, `skills-router`, `find-skills`, `init-brain`, `commit-push`, Kenmark `issues-*` / `skills-*` skills (including `skills-init`)
...
2. Suggest **skills-install-recommended** if they want a minimal curated set
