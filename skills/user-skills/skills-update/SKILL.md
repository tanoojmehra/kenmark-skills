...
Pair with **skills-maintain** if the machine has duplicate or stale skill trees before updating.
...
1. Confirm Kenmark skill folders exist (e.g. `~/.cursor/skills/skills-update`, `skills-router`, `init-brain`)
...
| Skill | Use when |
| --- | --- |
| **skills-maintain** | Audit duplicates before/after a large update |
| **skills-install-recommended** | First-time install of curated packs (not just refresh) |
| **skills-router** | Pick the right skill after the registry is current |

## Scripts

- `scripts/skills-update.js` — orchestrates npm + setup + install-recommended + adopt
...
- `scripts/skills-install-recommended.js` — curated packs (invoked by update)
