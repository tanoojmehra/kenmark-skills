...
Pairs with **skills-maintain** for the parallel skills cleanup and **skills-update** when refreshing Kenmark installs.
...
**Pair with `skills-maintain`** to do the same audit for skills — both inventories use the same output layout.
...
- `architect`, `build-error-resolver`, `capacitor-expert`, `chief-of-staff`, `code-reviewer`, `database-reviewer`, `doc-updater`, `e2e-runner`, `expert-documenter-reviewer`, `go-build-resolver`, `go-reviewer`, `harness-optimizer`, `kotlin-build-resolver`, `kotlin-reviewer`, `loop-operator`, `nextjs-fullstack-expert`, `nodejs-expert`, `planner`, `python-reviewer`, `refactor-cleaner`, `research-problem-solver`, `security-reviewer`, `senior-dev-troubleshooter`, `tdd-guide`
...
2. Suggest **skills-maintain** if the skills tree is also bloated
